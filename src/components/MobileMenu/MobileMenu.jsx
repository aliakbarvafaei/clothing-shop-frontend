                    ...
                    myAccountDrop.map((item,index)=>{
                        if(user){
                            if(item.title==="Logout")
                                return <Link className="text-left text-[14px] py-[12px] hoverItem font-font font-normal" onClick={()=>{dispatch({ type: 'logout' });localStorage.setItem('token_user', JSON.stringify(''));setToastState(old=>addItemOnce(old.slice(),{title:"2",description:"Logout Successfully", key:Math.random()}));}} to={item.pathTo} key={index}>{item.title}</Link>
