...
function Cart(props) {
  ...
  function handleQuantity(str, item) {
    const newQuantity =
      str === "-" ? Number(item.quantity) - 1 : Number(item.quantity) + 1;
    if (newQuantity !== 0) {
      // setToastState(old=>addItemOnce(old.slice(),{
      //     title: "3",
      //     description: "", key:Math.random()
      ...
      setProductCart((old) =>
        updateItemOne(old.slice(), item, String(newQuantity))
      );
    }
  }
  ...
  return (
    <div>
      ...
      <div className={`${themeClass} px-total py-[50px]`}>
        {(productCart.length > 0 || productCart === "") && (
          <>
            <table className="w-[100%] table-fixed">
              <thead
                className={`border-b-solid border-b-[.5px] ${themeBorder}`}
              >
                <tr>
                      ...
                      {item ? (
                        ...
                        >
                          ...
                          <td className="md:hidden p-[12px] text-[24px]">
                            $
                            {(Number(item.price) * (100 - Number(item.off))) /
                              100}
                          </td>
                          <td className="md:hidden p-[25px] text-darkGray flex flex-col gap-[5px] items-center justify-center">
                            <div className="flex lg:justify-center">
                              <div
                                className={`mt-[10px] flex flex-row items-center justify-between border-[1px] border-solid w-[100px] ${themeBorder}`}
                              >
                                <div
                                ...
                                >
                                  <i
                                    className={`fa fa-caret-left p-[8px] h-[100%] border-r-[1px] ${themeBorder}`}
                                    aria-hidden="true"
                                  ></i>
                                </div>
                                ...
                                >
                                  <i
                                    className={`fa fa-caret-right p-[8px] border-l-[1px] ${themeBorder}`}
                                    aria-hidden="true"
                                  ></i>
                                </div>
                              </div>
            ...
            <div className="flex md:flex-col mdmin:flex-row justify-between items-center my-[20px] mr-[0px] w-[100%]">
              <div className="mdmin:w-[30%] md:w-[60%] flex flex-row justify-between items-center font-bold">
                ...
                <span className="text-[24px]">
                  ${Number(Math.round(totalPrice + "e" + 3) + "e-" + 3)}
                </span>
              </div>
        ...
        {productCart.length === 0 && Array.isArray(productCart) && (
